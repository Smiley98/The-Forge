...
{
    PsIn Out;
    ((Out).position = MulMat(projView,MulMat(worldMat,vec4(((In).position).xyz, 1.0))));
    ((Out).normal = normalize((MulMat(worldMat,vec4((In).normal, 0.0))).rgb));
    ((Out).pos = (MulMat(worldMat,vec4(((In).position).xyz, 1.0))).rgb);
