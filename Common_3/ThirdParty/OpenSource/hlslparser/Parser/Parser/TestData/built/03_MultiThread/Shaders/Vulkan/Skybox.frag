...
vec4 HLSLmain(VSOutput input1)
{
    ...
    if((side == 1.0))
    {
        ...
        return texture(sampler2D(RightText, uSkyboxSampler), newtextcoord);
    }
    else if ((side == 2.0))
    {
        (newtextcoord = ((((input1).TexCoord).zy / vec2(20)) + vec2(0.5)));
        (newtextcoord = vec2((newtextcoord).x, (float(1) - (newtextcoord).y)));
        return texture(sampler2D(LeftText, uSkyboxSampler), newtextcoord);
    }
    else if ((side == 4.0))
    {
        (newtextcoord = ((((input1).TexCoord).xz / vec2(20)) + vec2(0.5)));
        (newtextcoord = vec2((newtextcoord).x, (float(1) - (newtextcoord).y)));
        return texture(sampler2D(BotText, uSkyboxSampler), newtextcoord);
    }
    else if ((side == 5.0))
    {
        (newtextcoord = ((((input1).TexCoord).xy / vec2(20)) + vec2(0.5)));
        (newtextcoord = vec2((newtextcoord).x, (float(1) - (newtextcoord).y)));
        return texture(sampler2D(FrontText, uSkyboxSampler), newtextcoord);
    }
    else if ((side == 6.0))
    {
        (newtextcoord = ((((input1).TexCoord).xy / vec2(20)) + vec2(0.5)));
        (newtextcoord = vec2((float(1) - (newtextcoord).x), (float(1) - (newtextcoord).y)));
        return texture(sampler2D(BackText, uSkyboxSampler), newtextcoord);
    }
    else
    {
        (newtextcoord = ((((input1).TexCoord).xz / vec2(20)) + vec2(0.5)));
        (newtextcoord = vec2((newtextcoord).x, (newtextcoord).y));
        return texture(sampler2D(TopText, uSkyboxSampler), newtextcoord);
    }
}
void main()
{
