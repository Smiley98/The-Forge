...
layout(set = 0, binding = 1) uniform texture1DArray uTex0;
...
vec4 HLSLmain(PsIn In)
{
    ...
    vec3 coord1 = vec3((uvw).yz, ((((In).albedo).w * float(3)) + float(0)));
    vec3 coord2 = vec3((uvw).zx, ((((In).albedo).w * float(3)) + float(1)));
    vec3 coord3 = vec3((uvw).xy, ((((In).albedo).w * float(3)) + float(2)));
