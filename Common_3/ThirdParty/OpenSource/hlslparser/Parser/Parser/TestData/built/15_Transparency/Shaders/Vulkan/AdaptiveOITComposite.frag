...
layout(set = 0, binding = 5) uniform utexture2D AOITClearMaskSRV;
layout(row_major, set = 0, binding = 6) buffer AOITDepthDataSRV
{
    AOITDepthData AOITDepthDataSRV_Data[];
};

layout(row_major, set = 0, binding = 7) buffer AOITColorDataSRV
{
    AOITColorData AOITColorDataSRV_Data[];
};
...
{
    uvec2 dim;
    GetDimensions1(AOITClearMaskSRV,dim[0],dim[1]);
...
{
    uint data = texelFetch(AOITClearMaskSRV, ivec2(pixelAddr).xy, 0);
    AOITLoadControlSurface(data, surface);
}
void AOITLoadDataSRV(in uvec2 pixelAddr, out AOITNode nodeArray[4])
{
    AOITData data;
    uint addr = AOITAddrGenSRV(pixelAddr);
    ((data).color = uvec4(AOITColorDataSRV_Data[addr]));
    [[unroll]] 
...
vec4 HLSLmain(VSOutput input1)
{
    ...
    if((!(ctrlSurface).clear))
    {
        ...
        {
            (color += (vec3(trans) * UnpackRGB((nodeArray[i]).color)));
            (trans = ((nodeArray[i]).trans / float(255)));
        }
        (outColor = vec4(color, ((nodeArray[(4 - 1)]).trans / float(255))));
    }
    return outColor;
}
void main()
